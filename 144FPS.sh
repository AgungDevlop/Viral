...
# FPS Injector 144FPS - Optimize device for high refresh rate gaming
...
# Display header
cmd notification post -S bigtext -t 'FPS INJECTOR 144FPS' 'Tag' 'Starting Configuration.' 2>/dev/null || echo -e "${RED}[!] Notification failed, continuing...${RESET}"
echo -e "${CYAN}█▓▒▒░░░ FPS INJECTOR 144FPS by Agung Developer ░░░▒▒▓█${RESET}"
echo ""
sleep 0.5
...
echo -e "${CYAN}═══════════════════════════════════════${RESET}"
echo ""
echo -e "${CYAN}█▓▒▒░░░ WELCOME TO 144FPS INSTALLATION ░░░▒▒▓█${RESET}"
echo ""
sleep 0.5

# Optimize Refresh Rate and FPS Settings to 144Hz
(
    settings put system user_refresh_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set user_refresh_rate${RESET}"
    settings put system min_refresh_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set min_refresh_rate${RESET}"
    settings put system max_refresh_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set max_refresh_rate${RESET}"
    settings put system peak_refresh_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set peak_refresh_rate${RESET}"
    settings put system display_refresh_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set display_refresh_rate${RESET}"
    ...
    setprop debug.hwui.refresh_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set debug.hwui.refresh_rate${RESET}"
    ...
    settings put system game_driver_min_frame_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set game_driver_min_frame_rate${RESET}"
    settings put system game_driver_max_frame_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set game_driver_max_frame_rate${RESET}"
    settings put system game_driver_vsync_enable 0 2>/dev/null || echo -e "${RED}[!] Failed to set game_driver_vsync_enable${RESET}"
    settings put system game_driver_fps_limit 144 2>/dev/null || echo -e "${RED}[!] Failed to set game_driver_fps_limit${RESET}"
    ...
    setprop debug.graphics.game_default_frame_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set debug.graphics.game_default_frame_rate${RESET}"
    ...
    settings put system thermal_limit_refresh_rate 144 2>/dev/null || echo -e "${RED}[!] Failed to set thermal_limit_refresh_rate${RESET}"
) &
...
sleep 0.5
echo -e "${GREEN}[✓] 144Hz Refresh Rate Applied${RESET}"
sleep 0.5
...
echo -e "${CYAN}═══════════════════════════════════════${RESET}"
echo ""
echo -e "${YELLOW}‼️ ENJOY YOUR OPTIMIZED 144FPS GAMING EXPERIENCE ‼️${RESET}"
echo ""
sleep 0.5
...
echo ""
sleep 0.5
echo -e "${CYAN}█▓▒▒░░░ THANKS FOR USING FPS INJECTOR 144FPS ░░░▒▒▓█${RESET}"
echo ""
cmd notification post -S bigtext -t 'FPS INJECTOR 144FPS' 'Tag' 'SUCCESSFULLY ACTIVATED.' 2>/dev/null || echo -e "${RED}[!] Notification failed${RESET}"
